...
If you finish lab early consider:
  ...
  - **SUPER BONUS** - Once you finish this whole lab, add a second model for comments, it should have the name of the person who wrote it, and some text for the comment (maybe time stamps?). This model should 'belong' the the post, the data should be related in some way. Do you own research of how to set up a `one-to-many` relationship (one post can have many comments, one comment only belongs to one post), in MongoDB. Use Mongo Documents, Google, or [Billies's Notes]https://git.generalassemb.ly/Flex-322/instructor-examples/tree/master/5-24-ExpressAPI) - note we will not have class time to teach a second, related model. 
